...
处理预览、`manifest.json` 和 `regions.json` 同样不提交。同步远端前，正文引用到的原图或处理图会通过
...
水印处理是可选步骤；当前文章的 `/uploads/articles/` 原图和 `/article-images/` 处理图均可直接上传 OSS 并同步远端。
同步时先校验、上传 OSS 并回写正文，再发送文章 JSON；校验或上传失败不会影响已完成的本地发布，
也不会向远端发送仍引用本地文件的正文。OSS 上传成功但远端失败时保留 OSS 地址，重试不重复上传这些图片。
原图和成品仍保留在本地，需自行备份；OSS 保存正文实际引用的版本，直接上传不会修改图片或自动加水印。
...
系统才会检查图片归属、文件名、格式和大小，将当前文章的原图或处理后图片上传 OSS，替换正文为固定的 OSS HTTPS 地址并保存。水印处理不是上传的前置条件，已有 OSS 图片直接复用；外链或 Blob 图片仍需先通过编辑器导入。
若上传期间文章被修改，则停止本次同步，避免覆盖新编辑内容。检查和上传完成后只发送文章 JSON；
远端接口按文章 ID 新增或更新内容，仍严格拒绝本地图片地址、普通外链和 Blob，只接收已转换的 OSS 地址，不接收或保存图片文件。
...
需要管理员会话，可查询全部文章，按标题搜索、按发布状态筛选，并按修改时间、创建时间、PV 或 UV 排序，每页 20 条。
列表显示创建/修改时间（北京时间）、访问次数（PV）、匿名访客数（UV）、最近访问、访问状态和剩余额度。支持访问明细、访问量管理、编辑和打开文章。

默认每篇文章累计 UV 额度为 10，PV 不限。第 10 位访客获准阅读后，后续打开文章会弹窗提示“访问受限”，正文由服务端拦截。后台点击“解除限制”，填写剩余可访问 UV（例如 5）并保存即可继续放行；PV 可同时设置，任一额度用完即再次限制。UV、PV 均可设为不限，填写 0 则立即限制。已有文章也采用默认额度，历史未识别访问每次占用 1 份 UV 额度。
...
新上传支持加密 HLS、访问码观看及加密资源远端同步，旧公开 MP4 不迁移。
启用前请阅读 [配置与操作说明](docs/private-videos.md)，生成每个部署独立的密钥并检查 OSS 权限。默认不启用新上传，不会回退到公开上传。
