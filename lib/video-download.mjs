...
import { writeFile, statfs, readFile, lstat, rename } from 'node:fs/promises';
...
export class VideoTailError extends VideoError {
  constructor(message, recovery) { super(message); this.recovery = recovery; }
}
const TAIL_RECORD = 'tail-recovery.json';
const tailFailure = error => error instanceof VideoError && /网络连接|传输中断|请求超时|HTTP (?:404|410|50[234])|AES-128 分片长度无效|分片不是|视频分片为空/.test(error.message);
...
export async function downloadVideo(url, directory, { signal, progress, fetcher = fetchResource, run = command, budget = { bytes: 0 } }) {
  ...
  async function manifest(value, depth = 0, allowTail = true) {
    ...
    if (playlist.kind === 'master') {
      const video = await manifest(playlist.url, depth + 1, allowTail && !playlist.audio);
      if (playlist.audio) {
        const audio = await manifest(playlist.audio, depth + 1, false);
    ...
    try {
      for (let i = 0; i < playlist.resources.length; i++) {
        ...
        try {
          ...
          signal.throwIfAborted();
          content = encrypted ? decryptFragment(fragment.bytes, keys.get(encrypted.key), encrypted.iv) : fragment.bytes;
          if (!content.length) throw new VideoError('视频分片为空。');
          extension = segmentExtension(content);
        } catch (error) {
          // Never skip a key/auth/quota error, a middle segment, an init map, or
          // one track of an externally paired audio/video presentation.
          if (!signal.aborted && allowTail && prefix === 'track-0' && i > 0 && i === playlist.resources.length - 1 && tailFailure(error)) {
            const last = playlist.output.findIndex(line => typeof line === 'object' && line.segment === i);
            let cut = last - 1;
            while (cut >= 0 && !(typeof playlist.output[cut] === 'string' && playlist.output[cut].startsWith('#EXTINF:'))) cut--;
            const missingSeconds = cut >= 0 ? Number(playlist.output[cut].slice(8).split(',')[0]) : 0;
            const keptDuration = playlist.duration - missingSeconds;
            if (last >= 0 && missingSeconds > 0 && keptDuration > 0) {
              const retained = playlist.output.slice(0, cut);
              const durations = retained.filter(line => typeof line === 'string' && line.startsWith('#EXTINF:')).map(line => Number(line.slice(8).split(',')[0]));
              const shortened = ['#EXTM3U', '#EXT-X-VERSION:7', `#EXT-X-TARGETDURATION:${Math.ceil(Math.max(...durations))}`, '#EXT-X-MEDIA-SEQUENCE:0', '#EXT-X-PLAYLIST-TYPE:VOD',
                ...retained.flatMap(line => typeof line !== 'string' ? ['map' in line ? `#EXT-X-MAP:URI="${filenames[line.map]}"` : filenames[line.segment]]
                  : line.startsWith('#EXTINF:') ? [`#EXTINF:${Number(line.slice(8).split(',')[0])},`] : line === '#EXT-X-DISCONTINUITY' ? [line] : []),
                '#EXT-X-ENDLIST'].join('\n') + '\n';
              const recovery = { missingSeconds, originalDuration: playlist.duration, keptDuration, downloaded, total, retainedAt: Date.now() };
              const record = { version: 1, ...recovery, playlist: shortened, files: retainedFiles };
              await writeFile(path.join(directory, `${TAIL_RECORD}.tmp`), JSON.stringify(record), { mode: 0o600 });
              await rename(path.join(directory, `${TAIL_RECORD}.tmp`), path.join(directory, TAIL_RECORD));
              throw new VideoTailError(`${error.message} 最后一个视频分片失败；已保留前面的分片，可手动确认忽略尾段后合成。`, recovery);
            }
          }
          throw error;
        }
        ...
        retainedFiles.push({ name: filename, bytes: content.length, sha256: hashBytes(content) });
        downloaded++;
  ...
  const result = await muxLocalVideo(source, directory, { signal, progress, run });
  return { ...result, bytes, downloaded, total };
}

async function muxLocalVideo(source, directory, { signal, progress, run = command }) {
...
/** Resume only server-produced, hash-verified local fragments; no network or keys. */
export async function recoverVideoTail(directory, { signal, progress, run = command }) {
  signal.throwIfAborted();
  const folder = await lstat(directory);
  if (!folder.isDirectory() || folder.isSymbolicLink()) throw new VideoError('保留分片目录无效。');
  const recordPath = path.join(directory, TAIL_RECORD), info = await lstat(recordPath);
  if (!info.isFile() || info.isSymbolicLink() || info.size > 4 * 1024 ** 2) throw new VideoError('尾段恢复记录无效。');
  let record;
  try { record = JSON.parse(await readFile(recordPath, 'utf8')); } catch { throw new VideoError('尾段恢复记录无法读取。'); }
  const { files, playlist, keptDuration, missingSeconds, originalDuration, downloaded, total } = record;
  if (record.version !== 1 || !Array.isArray(files) || !files.length || files.length >= LIMITS.files ||
      !Number.isInteger(downloaded) || downloaded !== files.length || total !== downloaded + 1 ||
      ![keptDuration, missingSeconds, originalDuration].every(value => Number.isFinite(value) && value > 0) ||
      originalDuration > LIMITS.duration || Math.abs(keptDuration + missingSeconds - originalDuration) > 0.001 ||
      typeof playlist !== 'string' || Buffer.byteLength(playlist) > LIMITS.playlist) throw new VideoError('尾段恢复记录不完整，不能合成。');
  const base = 'https://retained.invalid/';
  const parsed = parsePlaylist(playlist, base);
  if (parsed.kind !== 'media' || parsed.encryption.some(Boolean) || parsed.resources.length !== files.length || Math.abs(parsed.duration - keptDuration) > 0.001) throw new VideoError('保留播放清单不匹配。');
  let retainedBytes = 0;
  for (let i = 0; i < files.length; i++) {
    signal.throwIfAborted();
    const file = files[i];
    if (!file || !new RegExp(`^track-0-${i}\\.(ts|mp4|aac)$`).test(file.name) || parsed.resources[i] !== `${base}${file.name}` ||
        !Number.isInteger(file.bytes) || file.bytes <= 0 || file.bytes > LIMITS.segment || !/^[a-f0-9]{64}$/.test(file.sha256)) throw new VideoError('保留分片信息无效。');
    const filePath = path.join(directory, file.name), stat = await lstat(filePath);
    if (!stat.isFile() || stat.isSymbolicLink() || stat.size !== file.bytes) throw new VideoError('保留分片缺失或大小改变，不能合成。');
    const content = await readFile(filePath);
    if (hashBytes(content) !== file.sha256 || !file.name.endsWith(`.${segmentExtension(content)}`)) throw new VideoError('保留分片已损坏，不能合成。');
    retainedBytes += content.length;
    if (retainedBytes > LIMITS.bytes) throw new VideoError('保留分片总大小超限。');
  }
  const space = await statfs(directory);
  if (space.bavail * space.bsize < retainedBytes + 64 * 1024 ** 2) throw new VideoError('磁盘空间不足，不能合成保留分片。');
  const localManifest = path.join(directory, 'recovered.m3u8');
  await writeFile(localManifest, playlist, { mode: 0o600 });
  const result = await muxLocalVideo({ file: localManifest, duration: keptDuration }, directory, { signal, progress, run });
  return { ...result, downloaded, total, incomplete: { missingSegments: 1, missingSeconds, originalDuration } };
}
