...
export function VideoPicker({ onClose, onInsert, articleVideoBaseUrl }: {
  onClose: () => void;
  onInsert: (video: { src?: string; assetId?: string; title: string }) => string | void;
  articleVideoBaseUrl?: string | null;
}) {
  ...
  return <dialog ref={dialog} className={styles.dialog} aria-labelledby={titleId} aria-describedby={introId} onCancel={event => { event.preventDefault(); onClose(); }}>
    ...
    <p className={styles.intro} id={introId}>新视频加密上传至 OSS 后插入正文，访客使用访问码观看。已上传的旧公开视频保持原样。视频在当前光标位置插入，并显示在右侧预览；保存或发布文章仍需手动操作。</p>
    ...
    <div className={styles.jobs}>{jobs.map(job => {
      ...
      const privateSource = published && publication.kind === "private" && isPrivateVideoId(publication.assetId);
      const validSource = privateSource || (published && publication.kind !== "private" && isArticleVideoUrl(publication.url, base));
    ...
    <p className={styles.footer}>只有明确确认后才会上传。新私密视频不提供公开播放地址，访问码由播放站点后台管理。关闭窗口不会取消已提交的上传任务。</p>
  </dialog>;
}
