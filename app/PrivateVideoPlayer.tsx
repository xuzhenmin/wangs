...
const ACCESS_EVENT = "private-video-access-changed";

async function fetchAccessStatus(signal?: AbortSignal): Promise<boolean> {
  const response = await fetch("/api/private-videos/access", { cache: "no-store", credentials: "same-origin", signal });
...
export default function PrivateVideoPlayer({ assetId, title = "私密视频", admin = false }: { assetId: string; title?: string; admin?: boolean }) {
  ...
  const checkAccess = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      const authorized = await fetchAccessStatus(signal);
      if (!signal?.aborted) setAccess(authorized ? "ready" : "required");
      return authorized;
    } catch (reason) {
      // A transient status/network error is not evidence that a valid code expired.
      if (!signal?.aborted) setError(reason instanceof Error ? reason.message : "网络连接失败，请重试。");
      return null;
    }
  }, [admin]);
  ...
  useEffect(() => {
    ...
    const update = () => { void checkAccess(controller.signal); };
    update();
    window.addEventListener(ACCESS_EVENT, update);
    return () => { controller.abort(); window.removeEventListener(ACCESS_EVENT, update); };
  ...
  useEffect(() => {
    ...
    const failure = async () => {
      ...
      try {
        ...
        const authorized = await fetchAccessStatus(controller.signal);
        if (disposed) return;
        if (!authorized) { setAccess("required"); setError("观看权限已失效，请重新输入有效访问码。"); }
        else setError("视频资源暂时无法读取，请重试；无需重新输入访问码。");
      } catch {
        if (!disposed) setError("网络或观看权限验证暂时不可用，请重试；无需重新输入访问码。");
      }
    };
    ...
    if (element.canPlayType("application/vnd.apple.mpegurl")) {
      ...
      void import("hls.js").then(({ default: Hls }) => {
        if (disposed) return;
        if (!Hls.isSupported()) { setError("当前浏览器暂不支持此视频格式，请使用新版 Safari 或 Chrome。"); return; }
        const hls = new Hls();
        destroy = () => hls.destroy();
  ...
  async function submitCode(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/private-videos/access", { method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code: code.trim() }) });
      const result = await response.json();
      if (!response.ok || result.authorized !== true) throw new Error(result.error || "访问码无效或已撤销。");
      setCode(""); setAccess("ready"); window.dispatchEvent(new Event(ACCESS_EVENT));
  ...
  async function logout() {
    ...
    try {
      ...
      setAccess("required"); window.dispatchEvent(new Event(ACCESS_EVENT));
    } catch (reason) { setError(reason instanceof Error ? reason.message : "网络连接失败，请重试。"); }
    finally { setBusy(false); }
  }
