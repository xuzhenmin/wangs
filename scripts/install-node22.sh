...
echo "手动安装项目依赖时，请在当前终端先执行："
echo "  export PATH=\"$NODE_LINK/bin:\$PATH\""
echo "  bash scripts/install-low-memory.sh"
echo "可先用 bash scripts/install-low-memory.sh --check 查看内存和 Swap。"
echo "然后运行：./scripts/start-local.sh"
